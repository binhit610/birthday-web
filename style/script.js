...
const cornerSurprise = document.getElementById("corner-surprise");
...
// === Popup nhắc nhập pass nếu để trống ===
...
function typeMessage() {
  if (msgIndex >= messages.length) {
    cornerSurprise.classList.add("show");
    return;
  }
...
window.addEventListener("load", () => {
  music.volume = 0.2;
  const tryPlay = () => {
    music.play().then(() => { musicBtn.textContent = "🔊"; isPlaying = true; })
      .catch(() => { musicBtn.textContent = "🔇"; isPlaying = false; });
  };
  tryPlay();
